...
```batch
@echo off
set "src=E:\server\webapps\ROOT\upload"
set "dest=F:\files"

set "t0=%time: =0%"
set "datestr=%date:~0,4%%date:~5,2%%date:~8,2%_%t0:~0,2%%t0:~3,2%%t0:~6,2%"

set "log=F:\logs\backup_%datestr%.log"

set "mysqldumpdir=C:\Program Files\MySQL\MySQL Server 5.7\bin\"
set "webdb=F:\database\yzk_web_%datestr%.sql"

echo -------------------------------------------------------------------------------
echo    chunshu      ::     server-backup-ver20210805
echo -------------------------------------------------------------------------------
echo ------------------------------------------------------------------------------- >> %log%
echo    chunshu      ::     server-backup-ver20210805 >> %log%
echo ------------------------------------------------------------------------------- >> %log%
echo 源目录：%src%
echo 源目录：%src% >> %log%
echo 目标目录：%dest%
...
echo 日志：%log% >> %log%
echo 备份时间：%date% %time%
echo 备份时间：%date% %time% >> %log%
echo.
echo. >> %log%
echo.
echo. >> %log%
echo %date% %time% ^> 开始文件备份...
echo %date% %time% ^> 开始文件备份... >> %log%
robocopy %src% %dest% /e /eta /it /tee /log+:%log%
echo %date% %time% ^> 文件备份完毕.
echo %date% %time% ^> 文件备份完毕. >> %log%
echo.
echo. >> %log%
echo %date% %time% ^> 开始数据库备份...
echo %date% %time% ^> 开始数据库备份... >> %log%
cd /d %mysqldumpdir%
mysqldump -h localhost -uroot -ppassword --log-error=%log% yzk_web > %webdb%
echo %date% %time% ^> 数据库备份完毕.
echo %date% %time% ^> 数据库备份完毕. >> %log%
```
...
<Task version="1.2" xmlns="http://schemas.microsoft.com/windows/2004/02/mit/task">
  <Triggers>
    <CalendarTrigger>
      <StartBoundary>2021-08-05T14:02:00</StartBoundary>
  ...
  <Actions Context="Author">
    <Exec>
      <Command>E:\server-backup\server-backup.bat</Command>
      <WorkingDirectory>E:\server-backup\</WorkingDirectory>
    </Exec>
  </Actions>
</Task>
```
...
`/log+`：将状态输出到日志文件(附加到现有日志中)。

```batch
C:\Users\LYML\Desktop>robocopy /?
-------------------------------------------------------------------------------
   ROBOCOPY     ::     Windows 的可靠文件复制
-------------------------------------------------------------------------------

  开始时间: 2021年8月5日 15:12:37
...
参看 [数据库/mysql/命令行](数据库/mysql/命令行.md)
