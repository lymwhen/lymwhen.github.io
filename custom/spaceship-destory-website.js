function Asteroids(startX, startY) {
    const colours = ["#F73859", "#14FFEC", "#00E0FF", "#FF99FE", "#FAF15D", "#FF4081"];
	if (!window.ASTEROIDS)
		window.ASTEROIDS = {
			enemiesKilled: 0
		};
	...
	this.pos = new Vector(startX || 100, startY || 100);
	this.lastPos = false;
	...
	function addParticles(startPos) {
		...
		for (var i = 0; i < amount; i++) {
			that.particles.push({
				dir: (new Vector(Math.random() * 20 - 10, Math.random() * 20 - 10))
					.normalize(),
				pos: startPos.cp(),
				cameAlive: time
			});
	...
	addEvent(this.canvas, 'mousedown', function(e) {
		e = e || window.event; /*var message = document.createElement('span'); message.style.position = 'absolute'; message.style.border = '1px solid #999'; message.style.background = 'white'; message.style.color = "black"; message.innerHTML = 'Press Esc to quit'; document.body.appendChild(message); var x = e.pageX || (e.clientX + document.documentElement.scrollLeft); var y = e.pageY || (e.clientY + document.documentElement.scrollTop); message.style.left = x - message.offsetWidth / 2 + 'px'; message.style.top = y - message.offsetHeight / 2 + 'px'; setTimeout(function () { try { message.parentNode.removeChild(message); } catch (e) { } }, 1000);8*/
	});
	...
	var randomParticleColor = function() {
		// return (['#FF4081', 'yellow'])[random(0, 1)];
        return colours[random(0, colours.length - 1)];
	};
    // 爆炸粒子
	this.ctx.drawParticles = function(particles) {
		...
		for (var i = 0; i < particles.length; i++) {
			...
			this.drawLine(particles[i].pos.x, particles[i].pos.y, particles[i].pos.x - particles[i].dir.x * 20, particles[i].pos.y - particles[i].dir.y * 20);
		}
		this.fillStyle = oldColor;
	};
    // 火焰
	...
	this.update = function() {
		...
		for (var i = this.bullets.length - 1; i >= 0; i--) {
			...
			var bulletVel = this.bullets[i].dir.setLengthNew(bulletSpeed * tDelta)
