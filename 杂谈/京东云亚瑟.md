...
```bash
root@JDBoxV2:~# dd if=/tmp/uboot-JDC_AX1800_Pro-AX6600_Athena-20240510.bin of=$(
blkid -t PARTLABEL=0:APPSBL -o device) conv=fsync
1280+0 records in
1280+0 records out
655360 bytes (640.0KB) copied, 0.048256 seconds, 13.0MB/s
root@JDBoxV2:~# dd if=/tmp/uboot-JDC_AX1800_Pro-AX6600_Athena-20240510.bin of=$(
blkid -t PARTLABEL=0:APPSBL_1 -o device) conv=fsync
1280+0 records in
1280+0 records out
...
2. 执行`USB命令.bat`
3. 输入com口（设备管理器查看 9008 com 端口），回车，第一次执行成功的输出；观察“设备管理器”那里会刷新两次，再次出现 9008
...
将新的 emmc 焊接到 emmc 转 tf 上，插入 USD 读卡器（可能一般读卡器也行），插入电脑。
...
最简单的方式是，打开 Diskgenius，它会提示分区表有问题，点击更正即可。

![image-20250121003029643](assets/image-20250121003029643.png)
